...
if [[ "$(uname)" == "Darwin" ]] && ! command -v pngpaste >/dev/null 2>&1; then
  if command -v brew >/dev/null 2>&1; then
    echo "→ Installing pngpaste (enables clipboard image intake)..."
    brew install pngpaste --quiet && echo "✓ pngpaste installed" || echo "⚠ pngpaste install failed — clipboard intake disabled"
  else
    echo "⚠ pngpaste not found (brew unavailable) — clipboard intake disabled"
  fi
fi
...
if ! grep -q "^CREATEYA_API_KEY=crya_sk" "$PROJECT_ROOT/.env"; then
  if [[ -t 0 ]]; then
    ...
    if [[ -n "$api_key" ]]; then
      if [[ ! "$api_key" =~ ^crya_sk_[a-f0-9]{32}$ ]]; then
...
mkdir -p \
  ...
  "$PROJECT_ROOT/createya/assets/models" \
...
User-created presets live in the installed `creative-director/presets/<type>/_custom/` skill directory.
...
echo "│  $ASSETS_PATH"
echo "│                                                               │"
echo "│  Как добавлять референсы:                                     │"
echo "│  • Перетащи фото модели    → models/<имя>/                    │"
echo "│  • Перетащи фото товара    → products/<название>/             │"
echo "│  • Перетащи фото локации   → locations/<название>/            │"
echo "│                                                               │"
echo "│  После того как положил файл — скажи агенту:                  │"
echo "│  «закинул фото модели Sarah» или «добавил товар»              │"
echo "│  Агент найдёт файл, опишет его и загрузит в Createya.         │"
echo "└─────────────────────────────────────────────────────────────┘"
echo ""
echo "Альтернатива: кинь файл прямо в чат — агент сам найдёт его"
echo "в iCloud Downloads или ~/Downloads и разложит по папкам."
echo ""
echo "Следующий шаг:"
echo "  Restart your agent and say what you want to create."
echo ""
echo "MCP setup (if not already done):"
echo "  curl -fsSL https://api.createya.ai/install | bash -s -- \$CREATEYA_API_KEY"
echo ""
