...
**Шаг 4 — Онбординг-визард.** После того как понял задачу и собрал референсы — запускай визард. Задавай вопросы **по одному**, с вариантами ответа. Жди выбора перед следующим вопросом.
...
Если ответ очевиден из контекста (юзер уже сказал «Instagram» или «studio light») — пропускай вопрос, отмечай выбор в уме, переходи к следующему.

---

### Вопросы визарда (по порядку)

**1/5 — Платформа / формат:**
```
A) Instagram — лента (4:5, 1080×1350)
B) Instagram — Stories / Reels (9:16, 1080×1920)
C) Маркетплейс — Ozon / WB / Amazon (1:1 или 3:4)
D) Свой вариант (лендинг, пресса, принт...)
```

**2/5 — Модель генерации:**
```
A) GPT Image 2 — дешевле, хорошо держит референс лица/товара
B) Nano Banana 2 — высокий фотореализм, детализация кожи и одежды
C) Nano Banana Pro — максимальное качество, сложные сцены
D) Свой вариант
```

**3/5 — Локация:**
```
A) Улица / город (реальная или AI-локация)
B) Студия / нейтральный фон
C) Интерьер (кафе, квартира, офис, лофт...)
D) Свой вариант
```
*Если выбор A или C — уточни одним вопросом: «Конкретное место или оставляю на моё усмотрение?»*

**4/5 — Освещение:**
```
A) Golden hour — тёплый закатный свет, мягкие тени
B) Overcast / natural — рассеянный дневной, без теней
C) Studio — контролируемый, три точки, чистый
D) Dramatic — контрастный, кьяроскуро, кинематографичный
```

**5/5 — Количество кадров:**
```
A) 1 — только эталон
B) 3 — эталон + 2 вариации
C) 6 — эталон + 5 вариаций
D) Свой вариант
```

---

После визарда — сводка и старт:

```
Отлично. Вот что делаем:
• Платформа: [выбор] → размер [WxH]
• Модель: [выбор]
• Локация: [выбор]
• Свет: [выбор]
• Серия: [N] кадров

Начинаем с эталонного кадра. Запускаю?
```

**Шаг 5 — Старт.** После «да» / «запускай» / «давай» — идёшь в генерацию.
